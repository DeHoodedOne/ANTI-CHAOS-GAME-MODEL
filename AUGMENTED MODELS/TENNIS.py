...
def surface_checker():
    league = driver.find_element(By.CSS_SELECTOR, '.tournamentHeaderDescription .tournamentHeader__country').text.split(' ')
    surfaces = ["CLAY", "GRASS", "HARD"]
    try:
        if surfaces[0] in league:
            driver.find_element(By.LINK_TEXT, "CLAY").click()
        if surfaces[1] in league:
            driver.find_element(By.LINK_TEXT, "GRASS").click()
        if surfaces[2] in league:
            driver.find_element(By.LINK_TEXT, "HARD").click()
    except (NoSuchElementException, ElementClickInterceptedException, ElementNotInteractableException, StaleElementReferenceException):
        pass
